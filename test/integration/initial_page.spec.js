...
describe("Launch initial empty page", () => {
	...
	it("Whats New dialog should show up with top & bottom inside the window", () => {
		...
		cy.get(page.ids.dialog.whatsNew)
			.should("be.visible")
			.contains("What's New")
			.then((el) => {
	...
	it("Menus should exist and be clickable, with appropriately enabled / disabled content", () => {
		...
		cy.get(page.ids.subMenu.file.open).should(
			"have.class",
			page.classes.menu.enabled
		);
		cy.get(page.ids.subMenu.file.close).should(
			"have.class",
			page.classes.menu.disabled
		);
