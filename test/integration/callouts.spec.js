	...
	it("Custom Rotation on CSI, with correct Rotate Icon", () => {
		...
		cy.queryLic((lic) => {
			...
			assert.deepEqual(steps[0].model, {
				filename: "20015 - Alligator.mpd",
				parentStepID: null,
			});
			assert.deepEqual(steps[1].model, {
				filename: "20015 - Alligator.mpd",
				parentStepID: null,
			});
		...
		cy.queryLic((lic) => {
			...
			assert.deepEqual(steps[2].model, {
				filename: "20015 - Alligator.mpd",
				parentStepID: null,
			});
			assert.deepEqual(steps[0].parts, []);
		...
		cy.get("#select_part_0_cmenu").click();
		cy.queryLic((lic) => {
			assert.deepEqual(lic.app.selectedItemLookup, {
				id: 0,
				stepID: 44,
				type: "part",
			});
