describe("Test Page Views", () => {
	...
	it("Verify single page, side by side and scrolling page views", () => {
		...
		cy.get("#rightSubPane canvas")
			...
			.first()
			.should("have.id", "");
		cy.get("#rightSubPane canvas").last().should("have.id", "pageCanvas_43");

		cy.get("#rightSubPane").trigger("keyup", { key: "PageDown" });
		cy.get("#rightSubPane canvas")
			.should("have.length", 2)
			.first()
			.should("have.id", "pageCanvas_1");
		cy.get("#rightSubPane canvas").last().should("have.id", "pageCanvas_2");

		// One page scrolling
		cy.get("#view_menu").click();
		cy.get("#show_pages_menu").click();
		cy.get("#show_one_scroll_menu").click();
		cy.get("#rightSubPane canvas").should("have.length", 43);

		// Facing pages scrolling
		cy.get("#view_menu").click();
		cy.get("#show_pages_menu").click();
		cy.get("#show_two_scroll_menu").click();
		cy.get("#rightSubPane canvas").should("have.length", 44);
	});
});
