...
describe("Test multi book ", () => {
	function localSaveAs(blob, filename) {
		...
		JSZip.loadAsync(blob).then((zip) => {
			...
			zip.files[fileBaseName + "1.lic"].async("string").then((content) => {
			...
			zip.files[fileBaseName + "2.lic"].async("string").then((content) => {
				const file = JSON.parse(content);
				assert.equal(file.state.books.length, 1);
				assert.deepEqual(
					file.state.books[0].pages,
	...
	it("Multi-book dialog shows up with correct defaults", () => {
		...
		cy.getByTestId("multi-book-no-split-submodels", " input").should(
			"be.checked"
		);
		cy.getByTestId("multi-book-page-start-1", " input").should("be.checked");
		cy.getByTestId("multi-book-page-start-old", " input").should(
			"not.be.checked"
		);
		cy.getByTestId("multi-book-one-lic-file", " input").should("be.checked");
		cy.getByTestId("multi-book-many-lic-files", " input").should(
			"not.be.checked"
		);
	});
	...
	it("Split Alligator with default Multi-book settings", () => {
		...
		cy.queryLic((lic) => {
			...
			assert.equal(
				state.pages.filter((p) => p.subtype === "titlePage").length,
				2,
				"Have 2 title pages"
			);
		});
		...
		cy.get("#treeParent_book_0 > .treeChildren")
			.children()
			.should("have.length", 19);

		cy.get("#treeParent_book_1").should("exist");
		cy.get("#treeParent_book_1").click();
		cy.get("#treeParent_book_1 > .treeChildren")
			.children()
	...
	it("Split Alligator with no title pages", () => {
		...
		cy.queryLic((lic) => {
			...
			assert.equal(
				lic.app.currentPageId,
				43,
				"Selected page is the first title page"
			);
