...
function req(partName) {
	...
	if (partName.startsWith('48/')) {
		...
		path = partPathLookup[partPaths[partName] || 0];
	}
...
function parsePart(abstractPart, line) {
	var partName = line.slice(14).join(' ');
	var color = colorTable.get(line[1], 'color');
	color = forceBlack(color, abstractPart.name, partName);
	abstractPart.parts.push({
		color: color,
		name: line[14],
		matrix: line.slice(2, 14).map(parseFloat),
...
function parseLine(abstractPart, line) {
	abstractPart.primitives.push({
		...
		color: colorTable.get(line[1], 'edge'),
		points: [
...
function parseFace(abstractPart, line) {
	abstractPart.primitives.push({
		shape: primitiveTypes[line[0]],
		color: colorTable.get(line[1], 'color'),
		points: line.slice(2).map(parseFloat)
	});
}
...
function loadSubModels(lineList) {
	var models = [], lastFileLine, i, partName;
	...
	if (models.length) {
		...
		var lines = lineList.slice(models[0].start, models[0].end + 1);
...
function loadLDConfig() {
	var content = ajax('ldraw/LDConfig.ldr');
	var lineList = content.split('\n');
	for (var i = 0; i < lineList.length; i++) {
		var line = lineList[i].trim().replace(/\s\s+/g, ' ').split(' ');
	...
	if (colorTable[16]) {
		colorTable[16].color = -1;
		colorTable[16].edge = -1;
	}
	if (colorTable[24]) {
		colorTable[24].color = -1;
		colorTable[24].edge = -1;
	}
}
