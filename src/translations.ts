...
function translate(key: string, ...args: any[]): string {
	...
	if (key.startsWith("ctrl+")) {
		return (
			translate("glossary.ctrl") +
			" + " +
			key.charAt(key.length - 1).toUpperCase()
		);
	}
	...
	if (currentLocale && currentLocale !== "en") {
		try {
			...
			console.log(`Locale ${currentLocale} missing translation key: ${key}`);
			res = null;
		}
	}
	if (res == null) {
...
export { getLocale, LanguageList, noTranslate, setLocale, translate as tr };
