...
const Layout: LayoutInterface = {
	...
	titlePage(item) {
		...
		let titleAnnotation = annotations.find(
			(a) => a.meta.type === "title-page-model-name"
		);
		...
		let pageCountAnnotation = annotations.find(
			(a) => a.meta.type === "title-page-page-count"
		);
		...
		pageCountAnnotation.y =
			step.y + step.height + (step.y - pageCountAnnotation.height) / 2;
	...
	inventoryPage(page, box) {
		...
		pliItems.forEach(Layout.pliItem);
		...
		const margin =
			(getMargin(store.state.template.page.innerMargin) + qtyLabelOffset) * 1.2;
		let colWidth = 0,
			x = margin,
			y = margin;
		let columns: PLIItem[][] = [[]];
		...
		for (let i = 0; i < pliItems.length; i++) {
			...
			if (y + pliItem.height > box.height) {
				// Check if new item fits below current item
				x += colWidth + margin;
				y = margin;
				colWidth = 0;
				columns.push([]);
		...
		if (columns.length === 2) {
			...
			if (lastCol != null) {
				const remainingSpace =
					box.width - lastCol[0].x - Math.max(...lastCol.map((el) => el.width));
	...
	pageNumber(page) {
		...
		const lblSize = _.measureLabel(
			template.numberLabel.font,
	...
	step(step, box, pageMargin) {
		...
		pageMargin =
			pageMargin == null
				? getMargin(store.state.template.page.innerMargin)
				: pageMargin;
		const margin = getMargin(template.innerMargin);
		...
		const submodelBox = _.cloneDeep(box);
		submodelBox.x = submodelBox.y = 0;
		submodelBox.width = step.width;
		submodelBox.height = step.height;

		step.submodelImages.forEach((submodelImageID) => {
			const submodelImage = store.get.submodelImage(submodelImageID);
			Layout.submodelImage(submodelImage, submodelBox);
			_.geom.moveBoxEdge(submodelBox, "left", submodelImage.width + margin);
			const newY = submodelBox.y + submodelImage.height + margin;
			if (box.y < newY) {
				_.geom.moveBoxEdge(box, "top", newY - box.y);
			}
		});

		const pli =
			step.pliID != null && store.state.plisVisible
				? store.get.pli(step.pliID)
				: null;
		if (pli) {
			Layout.pli(pli);
			pli.y = box.y;
		...
		if (step.rotateIconID != null && csi != null) {
			...
			if (step.numberLabelID != null) {
				...
				if (
					...
					icon.x >= stepLabel.x &&
					icon.x <= stepLabel.x + stepLabel.width + margin
				) {
					icon.y = stepLabel.y + stepLabel.height + margin;
				}
			}
		...
		if (store.get.stepHasSubmodel(step)) {
			...
			let annotation = store.state.annotations.find(
				(a) => a.tagName === tagName
			);
			if (store.state.template.pli.includeSubmodels) {
				...
				if (prevStep && prevStep.parent.id === step.parent.id) {
					...
					while (annotation.points.length > 2) {
						store.mutations.item.delete({
							item: { type: "point", id: annotation.points[2] },
						});
					}
	...
	submodelImage(submodelImage, box) {
		...
		if (csi.scale != null) {
			// If user chose a manual scale factor, respect it
			csiSize = store.render.pli(part.colorCode, part.filename, csi);
		} else {
			csi.autoScale = template.csi.scale;
			csiSize = store.render.pli(part.colorCode, part.filename, csi);
			if (csiSize != null && csiSize.width > box.width * template.maxHeight) {
				csi.autoScale =
					((csi.autoScale ?? 1) * (box.width * template.maxHeight)) /
					csiSize.width;
		...
		submodelImage.width =
			borderWidth + margin + csiSize.width + margin + borderWidth;
		submodelImage.height =
			borderWidth + margin + csiSize.height + margin + borderWidth;
	...
	pliItem(pliItem) {
		...
		const pliSize = store.render.pli(
			pliItem.colorCode,
			pliItem.filename,
			pliItem
		);
	...
	callout(callout, box) {
		...
		if (callout.layout === "horizontal") {
			...
			stepSizes.forEach((entry, idx) => {
				const stepWidth =
					borderWidth + stepBox.x + entry.width + margin + borderWidth;
			...
			stepSizes.forEach((entry, idx) => {
				const stepWidth =
					borderWidth + stepBox.y + entry.height + margin + borderWidth;
	...
	calloutArrow(callout) {
		...
		while (callout.calloutArrows.length > 1) {
			store.mutations.item.delete({
				item: { type: "calloutArrow", id: callout.calloutArrows[1] },
			});
		}
		...
		while (arrow.points.length > 2) {
			store.mutations.item.delete({
				item: { type: "point", id: arrow.points[1] },
			});
		}
		...
		if (callout.steps.length > 1) {
			...
			if (
				lastStep != null &&
				callout.height - margin - lastStep.height - margin < 10
			) {
	...
	adjustBoundingBox: {
		...
		callout(item) {
			...
			Layout.adjustBoundingBox.item(
				callout,
				boxes,
				store.state.template.callout
			);
			Layout.calloutArrow(callout);
		},
...
function measureStep(step: Step) {
	...
	if (step.numberLabelID != null) {
		const lblSize = _.measureLabel(
			template.callout.step.numberLabel.font,
			step.number.toString()
		);
...
export function alignStepContent(page: Page) {
	...
	if (
		steps.length < 2 ||
		typeof page.actualLayout !== "object" ||
...
function isStepTooSmall(stepID: number) {
	...
	const csi = store.get.csi(step.csiID);
	const pli =
		store.state.plisVisible && step.pliID != null
			? store.get.pli(step.pliID)
			: null;
	...
	step.submodelImages.forEach((submodelImageID) => {
		const submodelImage = store.get.submodelImage(submodelImageID);
		submodelSpace.width = Math.max(
			submodelImage.width * 1.05,
			submodelSpace.width
		);
	...
	if (step.width < csi.width * 1.1) {
		...
		step.height <
		(submodelSpace.height + pliHeight + csi.height) * 1.2
	) {
		return true;
	}
	return false;
