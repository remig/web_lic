...
(function loadUIState() {
	...
	const checkedItems: { [key: string]: boolean } = uiState.get(
		"navTree.checkedItems"
	);
...
function createNode(item: ItemTypes): HTMLLIElement | null {
	...
	if (childNodes.length) {
		...
		_.dom.createElement(
			"i",
			{ class: "treeIcon fas fa-lg fa-caret-right" },
			div
		);
		div.appendChild(textNode);

		const childContainer = _.dom.createElement(
			"ul",
			{ class: "treeChildren hidden" },
			div
		);
