...
// const centerOfMassPartName = 'lic_center_of_mass_box';
...
function generateObjectList(part, modelView, colorCode, config) {
	...
	if (buffers) {
		const edgeColorCode =
			colorCode === studFaceColorCode ? config.parentColorCode : colorCode;
		...
		if (buffers.faces) {
			...
			} else if (config.addedPartBorderColor) {
				addObject(res.newlyAddedFaces, buffers.faces, modelView, color);
				addObject(
					res.newlyAddedScaledFaces,
					buffers.faces,
					modelView,
					config.addedPartBorderColor
				);
			} else {
	...
	if (part.parts && part.parts.length) {
		if (config.isModel) {
			...
			const localPartList =
				config.partList == null
					? part.parts.map((p, idx) => idx)
					: config.partList;

			for (let i = 0; i < localPartList.length; i++) {
				let partBox;
				const localPartId = localPartList[i];
				const subPart = part.parts[localPartId];
				...
				const newColorCode =
					subPart.colorCode == null ? colorCode : subPart.colorCode;
				let localAlpha = config.alpha;
				if (
					config.selectedPartIDs &&
					config.selectedPartIDs.includes(localPartId)
				) {
				...
				const displacement = displacedParts[localPartId];
				if (displacement) {
					...
					const arrowMat = Arrows.getArrowPosition(
						partBox,
						partMatrix,
						displacement
					);
					...
					addArrowObject(
						res.faces,
						arrowRotMat,
						(displacement.arrowLength || 60) - 15
					);
				}

				let localAddedPartBorderColor = config.addedPartBorderColor;
				if (
					Array.isArray(config.partsAddedThisStep) &&
					config.partsAddedThisStep.length > 0 &&
					config.partsAddedThisStep.includes(localPartId)
				) {
					localAddedPartBorderColor = [1, 0.1294, 0.345, 1.0]; // rgb(255, 33, 88)
				}

				const localConfig = {
					alpha: localAlpha,
					addedPartBorderColor: localAddedPartBorderColor,
				...
				concatArray(newObject.newlyAddedFaces, res.newlyAddedFaces);
				concatArray(newObject.newlyAddedScaledFaces, res.newlyAddedScaledFaces);
			...
			// 	const comMatrix = LDMatrixToMatrix([...part.centerOfMass, 1, 0, 0, 0, 1, 0, 0, 0, 1]);
			// 	const color = [83 / 256, 142 / 256, 201 / 256, 1.0];
			// 	const buffer = partBufferCache[centerOfMassPartName];
			// 	addObject(res.lines, buffer, comMatrix, color);
			// }
		} else if (part.parts) {
			for (let i = 0; i < part.parts.length; i++) {
				...
				const localConfig = {
					alpha: config.alpha,
					addedPartBorderColor: config.addedPartBorderColor,
					parentColorCode: colorCode,
				};
...
function drawScene(objectsToDraw, config) {
	...
	// Draw newly added faces, with stencil buffer write enabled
	...
	gl.useProgram(programs.faces.program);
	programs.faces.uniformSetters.projection(projectionMatrix);
	for (let i = 0; i < objectsToDraw.newlyAddedFaces.length; i++) {
		const object = objectsToDraw.newlyAddedFaces[i];
		gl.bindVertexArray(object.buffers.vao);
		twgl.setUniforms(programs.faces, object.uniforms);
		gl.drawElements(
			gl.TRIANGLES,
	...
	// Draw newly added faces, slightly scaled up, and use the stencil buffer to mask out the inside bits
	gl.stencilFunc(gl.EQUAL, 0, 1);
	gl.stencilOp(gl.KEEP, gl.KEEP, gl.KEEP);
	gl.stencilMask(0x00);

	gl.useProgram(programs.faces.program);
	for (let i = 0; i < objectsToDraw.newlyAddedScaledFaces.length; i++) {
		const object = objectsToDraw.newlyAddedScaledFaces[i];
		const outlineProj = makeOutlineProjection(
			projectionMatrix,
			object.uniforms.modelView,
			1
		);
		programs.faces.uniformSetters.projection(outlineProj);
		gl.bindVertexArray(object.buffers.vao);
		twgl.setUniforms(programs.faces, object.uniforms);
		gl.drawElements(
			gl.TRIANGLES,
...
// Returns a copy of projMatrix with a uniform 2D scale applied in clip space,
// centered on the object's projected origin. For ortho projection w=1 throughout,
// so this is a simple scale-about-point in NDC.
function makeOutlineProjection(projMatrix, modelView, scale) {
	// Object origin in world space (translation column of modelView)
	const wx = modelView[12],
		wy = modelView[13],
		wz = modelView[14];
	...
	const cy =
		...
		projMatrix[9] * wz +
		projMatrix[13];
	// Build S_clip * projMatrix, where S_clip scales XY by `scale` about (cx, cy).
	// For each column j: out_row0 = scale*P_row0 + cx*(1-scale)*P_row3
	//                    out_row1 = scale*P_row1 + cy*(1-scale)*P_row3
	//                    out_row2 = P_row2,  out_row3 = P_row3
	const out = new Float32Array(16);
	const s = scale,
		ox = cx * (1 - s),
		oy = cy * (1 - s);
	for (let j = 0; j < 4; j++) {
		out[j * 4] = s * projMatrix[j * 4] + ox * projMatrix[j * 4 + 3];
		out[j * 4 + 1] = s * projMatrix[j * 4 + 1] + oy * projMatrix[j * 4 + 3];
		out[j * 4 + 2] = projMatrix[j * 4 + 2];
		out[j * 4 + 3] = projMatrix[j * 4 + 3];
	}
	return out;
}
...
function addLine(lineData, p, cp) {
	...
	lineData.indices.data.push(idx + 2, idx + 1, idx, idx + 3, idx + 1, idx + 2);
...
function getPartBoundingBox(part, modelView) {
		...
		for (let i = 0; i < part.primitives.length; i++) {
			...
			if (Array.isArray(p) && (p.length === 9 || p.length === 12)) {
				for (let j = 0; j < p.length; j += 3) {
					const pt = twgl.m4.transformPoint(modelView, [
						p[j],
						p[j + 1],
						p[j + 2],
					]);
	...
	if (part.parts && part.parts.length) {
		for (let i = 0; i < part.parts.length; i++) {
			...
			if (subBox.min[0] < minX) {
				minX = subBox.min[0];
			}
			if (subBox.min[1] < minY) {
				minY = subBox.min[1];
			}
			if (subBox.min[2] < minZ) {
				minZ = subBox.min[2];
			}
			if (subBox.max[0] > maxX) {
				maxX = subBox.max[0];
			}
			if (subBox.max[1] > maxY) {
				maxY = subBox.max[1];
			}
			if (subBox.max[2] > maxZ) {
				maxZ = subBox.max[2];
			}
		}
	}
...
function importPart(part) {
	if (
		partBufferCache[part.filename] == null &&
		part.primitives &&
...
export default {
	initialize: function () {
		...
		const faceShader = twgl.createShader(
			gl,
			faceShaderSource,
			gl.VERTEX_SHADER
		);
		const lineShader = twgl.createShader(
			gl,
			lineShaderSource,
			gl.VERTEX_SHADER
		);
		const condLineShader = twgl.createShader(
			gl,
			condLineShaderSource,
			gl.VERTEX_SHADER
		);
		const fragShader = twgl.createShader(
			gl,
			fragmentShaderSource,
			gl.FRAGMENT_SHADER
		);

		const faceProgram = twgl.createProgram(gl, faceShader, fragShader, [
			"position",
		]);
		...
		// partBufferCache[centerOfMassPartName] = Arrows.createCenterOfMassLinesBuffers(gl);
	...
	initModel: function (model) {
		if (model == null) {
			...
			LDParse.loadRemotePart(url).then(function () {
