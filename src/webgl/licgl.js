...
function generateObjectList(part, modelView, colorCode, config, res) {
	console.log(config);
	if (!res) {
		res = {
			faces: [],
	...
	if (buffers) {
		...
		if (buffers.faces) {
			...
			if (config.addedPartBorderColor) {
				addObject(
			...
			} else if (config.isNewlyAddedPart) {
		...
		if (buffers.lines && config.addedPartBorderColor == null) {
			addObject(res.lines, buffers.lines, modelView, edgeColor);
		}
		if (buffers.condLines && config.addedPartBorderColor == null) {
	...
	if (part.parts && part.parts.length) {
		if (config.isModel) {
			...
			for (let i = 0; i < localPartList.length; i++) {
				...
				let localIsNewlyAddedPart = config.isNewlyAddedPart;
				if (
					...
					localIsNewlyAddedPart = true;
				}

				const localConfig = {
					alpha: localAlpha,
					isNewlyAddedPart: localIsNewlyAddedPart,
				...
				if (
					...
					generateObjectList(
						...
						partMatrix,
						[1, 0.1294, 0.345, 1.0],
						{ ...localConfig, addedPartBorderColor: [1, 0.1294, 0.345, 1.0] },
						res
					);
				}
			}
			...
			for (let i = 0; i < part.parts.length; i++) {
				...
				const localConfig = {
					alpha: config.alpha,
					isNewlyAddedPart: config.isNewlyAddedPart,
					addedPartBorderColor: config.addedPartBorderColor,
					parentColorCode: colorCode,
				};
