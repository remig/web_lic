...
function generateObjectList(part, modelView, colorCode, config) {
		...
		if (buffers.coloredFaces) {
			for (const key in buffers.coloredFaces) {
				if (buffers.coloredFaces.hasOwnProperty(key)) {
					addObject(res.faces, buffers.coloredFaces[key], modelView, LDParse.getColor(key, 'rgba'));
				}
			}
		}
		if (buffers.lines) {
	...
	if (part.parts && part.parts.length) {
		if (config.isModel) {
			...
			const localPartList =
				config.partList == null ? part.parts.map((p, idx) => idx) : config.partList;
...
function addLine(lineData, p, cp) {
	...
	if (cp != null) {
		// prettier-ignore
		lineData.condPointA.push(cp[0], cp[1], cp[2], cp[0], cp[1], cp[2], cp[0], cp[1], cp[2], cp[0], cp[1], cp[2]);
		// prettier-ignore
		lineData.condPointB.push(cp[3], cp[4], cp[5], cp[3], cp[4], cp[5], cp[3], cp[4], cp[5], cp[3], cp[4], cp[5]);
	}
...
export default {
	...
	composeLDMatrix(transform) {
		...
		const y = _.radians(rot.y);
		const z = _.radians(rot.z);
		const a = Math.cos(x);
		const b = Math.sin(x);
		const c = Math.cos(y);
		const d = Math.sin(y);
