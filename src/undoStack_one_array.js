...
const api = {
	...
	commit(changeList, opts, undoText, clearCacheTargets) {
		...
		if (typeof changeList === 'string') {
			changeList = [{mutation: changeList}];
		} else if (isAction(changeList)) {
			changeList = [changeList];
		}

		changeList.forEach(change => {
			if (change.hasOwnProperty('mutation')) {
				const mutation = _.get(change.mutation, store.mutations);
				if (mutation) {
					mutation(opts);
				}
		...
		let newState;
		if (_.isEmpty(changeList.filter(el => el.hasOwnProperty('mutation')))) {
...
function isAction(el) {
	return el && el.hasOwnProperty('undo') && el.hasOwnProperty('redo');
}
