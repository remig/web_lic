	...
	window.setTimeout(() => {
		...
		exportPages(pages, canvas).then(() => {
			doneCallback(() => {
				...
				statusText.value = t(`action.export.${exportType.toLowerCase()}.success_message_@mf`, {
					exportType,
					time,
				});
			});
...
function generatePDF(store: Store, config?: PDFConfig) {
	...
	const doc = new jsPDF(pageSize.width > pageSize.height ? 'landscape' : 'portrait', 'pt', [
		pageSize.width,
		pageSize.height,
	]);
...
function generatePNGZip(store: Store, hiResScale: number = 1, dpi: number = 96) {
	...
	function drawPage(page: Page, canvas: HTMLCanvasElement) {
		...
		const pageName =
			page.subtype === 'titlePage' ? 'Page 1 Title Page.png' : `Page ${page.number}.png`;
