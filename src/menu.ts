...
function addGuide(orientation: Orientations) {
	return function () {
		...
		const position = Math.floor(
			orientation === "vertical" ? width / 2 : height / 2
		);
		const change = {
		...
		undoStack.commit(
			change,
			null,
			tr(`action.view.guides.add_${orientation}.undo`)
		);
	};
}
		...
		children: [
				...
				children: [
					...
					},
					{
						...
						cb() {
							...
							undoStack.commit("templatePage.reset", null, text, [
								"csi",
								"pliItem",
							]);
						},
			...
			},
			{
				...
				cb() {
					...
					Storage.clear.everything(uiState);
					app.redrawUI();
				},
			},
		],
	},
	{
		...
		children: [
			...
			},
			{
				...
				cb: () => DialogManager("brickColorsDialog"),
			},
		],
	},
	{
		text: "action.view.name",
		id: "view_menu",
		children: [
			{
			...
			},
			{
				...
				children: [
					{
						text: "action.view.zoom.full.name",
						id: "zoom_full_menu",
						enabled: () => false,
						cb() {},
					},
					{
						text: "action.view.zoom.fit.name",
						id: "zoom_fit_menu",
						enabled: () => false,
						cb() {},
					},
					{
						text: "action.view.zoom.in.name",
						id: "zoom_in_menu",
						enabled: () => false,
						cb() {},
					},
					{
						text: "action.view.zoom.out.name",
						id: "zoom_out_menu",
						enabled: () => false,
						cb() {},
					},
	...
	},
	{
		...
		children: [
			...
			},
			{
				...
				enabled: enableIfModel,
				cb() {
					DialogManager("exportPdfDialog", (dialog) => {
			...
			},
			{
				...
				enabled: enableIfModel,
				cb() {
					DialogManager("exportPngDialog", (dialog) => {
						dialog.$on("ok", (newValues: any) => {
							InstructionExporter.generatePNGZip(
								app,
								store,
								newValues.scale,
								newValues.dpi
