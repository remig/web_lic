		...
		children: [
			...
			},
			{
				...
				async cb() {
					...
					if (opts != null) {
						undoStack.commit('book.divideInstructions', opts, t('action.edit.multi_book.undo'));
						SelectionOps.setCurrentPage(store.get.firstPage());
					}
				},
			},
	...
	},
	{
		...
		children: [
			...
			},
			{
				...
				enabled: enableIfModel,
				async cb() {
					const result = await DialogManager.showPdfExportDialog({
						pageSizeInPixels: store.state.template.page,
					});
					if (result != null) {
			...
			},
			{
				...
				enabled: enableIfModel,
				async cb() {
					const result = await DialogManager.showPngExportDialog({
						pageSizeInPixels: store.state.template.page,
					});
					if (result != null) {
