...
const annotationMenu: ContextMenuEntryInterface = {
	...
	children: [
		{
			...
			cb(selectedItem) {
				...
				undoStack.commit(
					"annotation.add",
					opts,
					tr("action.annotation.add.label.undo")
				);
			},
		},
		{
		...
		},
		{
			...
			cb(selectedItem) {
				...
				undoStack.commit(
					"annotation.add",
					opts,
					tr("action.annotation.add.arrow.undo")
				);
			},
		},
		{
			...
			cb(selectedItem) {
				...
				openFileHandler(".png", "dataURL", (src) => {
					...
					undoStack.commit(
						"annotation.add",
						opts,
						tr("action.annotation.add.image.undo")
					);
				});
			},
		},
...
const contextMenu = {
				...
				},
				{
					...
					cb(selectedItem: LookupItem) {
						...
						undoStack.commit(
							"page.layout",
							opts,
							tr("action.layout.horizontal.undo")
						);
					},
				},
				{
					...
					cb(selectedItem: LookupItem) {
						...
						const values =
							typeof originalLayout === "string"
								? { rows: "auto", cols: "auto", direction: originalLayout }
								: {
										rows: originalLayout.rows,
										cols: originalLayout.rows,
										direction: originalLayout.direction,
									};

						DialogManager("pageLayoutDialog", (dialog) => {
							dialog.show({ values, page });
						});
					},
				},
			],
		...
		},
		{
			...
			cb(selectedItem: LookupItem) {
				...
				const opts = {
					...
					insertionIndex:
						prevStep == null ? 0 : store.state.steps.indexOf(prevStep) + 1,
				};
	...
	step: [
					...
					cb(selectedItem: LookupItem) {
						...
						DialogManager("pageLayoutDialog", (dialog) => {
							...
							if (typeof originalLayout === "string") {
								...
								dialog.values.direction =
									originalLayout.direction || "vertical";
							}
		...
		},
		{
			...
			cb(selectedItem: LookupItem) {
				undoStack.commit(
					"step.addSubStep",
					{ step: selectedItem, doLayout: true },
					tr(this.text)
				);
			},
		},
		{
			...
			cb(selectedItem: LookupItem) {
				...
				if (step.stretchedPages.length > 0) {
					page = store.get.page(
						step.stretchedPages[step.stretchedPages.length - 1]
					);
				} else {
			...
			children: [
				{
					...
					shown(selectedItem: LookupItem) {
						const page = store.get.pageForItem(selectedItem);
						if (
							store.get.isFirstBasicPage(page) ||
							store.get.isTitlePage(page)
						) {
				...
				},
				{
					...
					shown(selectedItem: LookupItem) {
						const page = store.get.pageForItem(selectedItem);
						if (
							store.get.isLastBasicPage(page) ||
							store.get.isTitlePage(page)
						) {
							return false; // Next page doesn't exist
						} else if (
							page.steps.indexOf(selectedItem.id) !==
							page.steps.length - 1
						) {
		...
		},
		{
			...
			shown(selectedItem: LookupItem) {
				...
				if (
					...
					store.get.callout(step).steps.length < 2
				) {
				...
				},
				{
					...
					cb(selectedItem: LookupItem) {
						...
						if (initialRotation == null) {
							initialRotation =
								store.get.templateForItem(selectedItem).rotation;
						}
			...
			children(selectedItem: LookupItem) {
				...
				if (step != null && isItemSpecificType(step, "step")) {
					return (step.parts || []).map((partID) => {
						const part = LDParse.model.get.partFromID(
							partID,
							step.model.filename
						);
	...
	pliItem: [
		{
			...
			cb(selectedItem: LookupItem) {
				...
				DialogManager("rotatePartImageDialog", (dialog) => {
					...
					dialog.$on("ok", () => {
						const dirtyItems = store.state.pliItems.filter(
	...
	annotation(selItem: LookupItem) {
		const deleteMenu: ContextMenuEntryInterface = {
			...
			cb(selectedItem) {
				...
				undoStack.commit(
					"annotation.delete",
					{ annotation },
					tr("action.annotation.delete.undo")
				);
				app.clearSelected();
			},
		};

		const setStyleMenu: ContextMenuEntryInterface = {
			...
			children: [
				{
					...
					cb(selectedItem: LookupItem) {
						...
						undoStack.commit(
							"callout.layout",
							opts,
							tr("action.callout.layout.horizontal.undo")
						);
					},
				},
				{
					...
					cb(selectedItem: LookupItem) {
						...
						undoStack.commit(
							"callout.layout",
							opts,
							tr("action.callout.layout.vertical.undo")
						);
					},
				},
			],
						...
						text:
							idx === 0
								...
								: idx === arrow.points.length - 1
									...
									: noTranslate(
											tr("action.callout_arrow.select_point.point.name_@mf", {
												idx,
											})
										),
	...
	submodel: [
		{
			...
			shown(selectedItem: LookupItem) {
				...
				if (isItemSpecificType(selectedItem, "submodel")) {
					const submodel = LDParse.model.get.abstractPart(
			...
			cb(selectedItem: LookupItem) {
				if (isItemSpecificType(selectedItem, "submodel")) {
					...
					if (destStep.id != null) {
						app.setCurrentPage(
							store.get.pageForItem({ type: "step", id: destStep.id })
						);
					}
				}
			},
	...
	part: [
		...
		},
		{
			...
			shown(selectedItem: LookupItem) {
				if (isItemSpecificType(selectedItem, "part")) {
					...
					if (step.displacedParts) {
						return step.displacedParts.some(
							(p) => p.partID === selectedItem.id
						);
					}
				}
				return false;
			...
			cb(selectedItem: LookupItem) {
				...
				const match = step.displacedParts.find(
					(p) => p.partID === selectedItem.id
				);
				if (match == null) {
					return;
						...
						DialogManager("ldColorPickerDialog", (dialog) => {
							dialog.$on("ok", (newColorCode) => {
								...
								const change = [
									action,
									{
										mutation: "pli.syncContent",
										opts: { pli, doLayout: true },
									},
								];
					...
					cb(selectedItem: LookupItem) {
						...
						DialogManager("stringChooserDialog", (dialog) => {
							dialog.$on("ok", (filename) => {
								LDParse.loadRemotePart(filename).then(
					...
					cb(selectedItem: LookupItem) {
						if (isItemSpecificType(selectedItem, "part")) {
							...
							undoStack.commit(
								changes,
								null,
								tr("action.part.change_part.duplicate.undo"),
								["csi"]
							);
						}
					},
					...
					cb(selectedItem: LookupItem) {
						if (isItemSpecificType(selectedItem, "part")) {
							...
							undoStack.commit(
								[mutation, action],
								null,
								tr("action.part.change_part.delete.undo"),
								["csi"]
							);
							app.clearSelected();
						}
		...
		if (isItemSpecificType(selectedItem, "part")) {
			...
			if (step.csiID != null) {
				...
				undoStack.commit(
					...
					tr("action.part.displace_part.undo_@mf", {
	...
	for (let i = 0; i < menu.length; i++) {
		...
		if (!isSeparator(entry)) {
			if (entry.shown && !entry.shown(selectedItem)) {
				...
				const children =
					typeof entry.children === "function"
...
export default function ContextMenu(selectedItem: LookupItem, localApp: any) {
	...
	const clickedType = subtype || selectedItem.type;
	if (
		clickedType === "book" ||
		clickedType === "submodelImage" ||
		clickedType === "templatePage"
	) {
		return null;
	}
	const tempMenu = contextMenu[clickedType];
	const menu =
		typeof tempMenu === "function" ? tempMenu(selectedItem) : tempMenu;
