...
export interface GetterInterface {
	...
	prev<T extends NumberedItem>(
		itemLookup: LookupItem,
		itemList?: NumberedItem[]
	): T | null;
	next<T extends NumberedItem>(
		itemLookup: LookupItem,
		itemList?: NumberedItem[]
	): T | null;
	...
	coords: {
		pageToItem({ x, y }: Point, itemLookup: LookupItem | null): Point;
...
export const Getters: GetterInterface = {
		...
		if (hasProperty<PLIItemParent>(parent, "pliItems")) {
			return (
				(parent.pliItems || []).map(store.get.pliItem).find((i) => {
					return (
						i && i.filename === part.filename && i.colorCode === part.colorCode
					);
				}) || null
			);
		}
	...
	prev<T>(itemLookup: LookupItem, itemList?: NumberedItem[]) {
		...
		const foo: NumberedItem[] =
			itemList || getStateChildList<NumberedItem>(item.type);
	...
	children(itemLookup: LookupItem, childTypeList?: ItemTypeNames[]) {
		...
		foo.forEach((childType: ItemTypeNames) => {
			...
			if (childIDList && childIDList.length) {
				children.push(
					...childIDList.map((id: number) => getItem(childType, id))
				);
			} else {
			...
			.forEach((step: Step) => {
				if (!addedModelNames.has(step.model.filename)) {
					const modelHierarchy = [
						{ filename: step.model.filename, quantity: 1 },
					];
					...
					while (parentStepID != null) {
						...
						if (parentStep.parts.length > 1) {
							...
							const partNames = parentStep.parts.map((partID: number) => {
								return LDParse.model.get.partFromID(
									partID,
									parentStep.model.filename
								).filename;
							});
	...
	topLevelTreeNodes() {
		...
		if (store.state.books.length) {
			return [store.get.templatePage(), ...store.state.books].filter(
				(el) => el
			);
		}
	...
	lookupToItem(
		...
		if (
			Object.prototype.hasOwnProperty.call(lookup, "parent") ||
			Object.prototype.hasOwnProperty.call(lookup, "number") ||
			Object.prototype.hasOwnProperty.call(lookup, "steps")
		) {
			return lookup as ItemTypes; // lookup is already an item
		} else if (Object.prototype.hasOwnProperty.call(store.state, lookup.type)) {
		...
		if (itemList) {
			return itemList.find((el: Item) => el.id === lookup.id) || null;
		}
		return null;
	},
	itemToLookup(item: Item) {
		// Create a {type, id} lookup object from the specified item
		if (!item || item.type == null) {
			return null;
		} else if (item.type === "part" || item.type === "submodel") {
			return item;
		} else if (Object.prototype.hasOwnProperty.call(store.state, item.type)) {
			return { type: item.type, id: item.id || 0 };
		} else if (
			!Object.prototype.hasOwnProperty.call(store.state, item.type + "s")
		) {
			return null;
		}
	...
	coords: {
		...
		itemToPage(itemLookup: LookupItem) {
