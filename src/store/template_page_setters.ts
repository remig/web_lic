...
export const TemplatePageMutations: TemplatePageMutationInterface = {
	async add() {
		...
		store.mutations.submodelImage.add({
			parent: step,
			modelFilename: modelData.model.filename,
			quantity: 2,
		});
		...
		const callout = store.mutations.callout.add({
			parent: step,
	...
	step: {
		csi: {
			scale({ scale }) {
				store.state.template.step.csi.scale = scale;
	...
	submodelImage: {
		csi: {
			scale({ scale }) {
				store.state.template.submodelImage.csi.scale = scale;
				store.get.templatePage().needsLayout = true;
				store.mutations.csi.markAllDirty();
				store.mutations.page.markAllDirty();
			},
