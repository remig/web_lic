...
export const CalloutMutations = {
	...
	addStep({
		...
		const destStep =
			item.steps[insertionIndex] == null ? null : store.get.step(item.steps[insertionIndex]);
		...
		const stepNumber = destStep ? destStep.number || 1 : lastStep.number + 1;
		...
		item.steps.map(store.get.step).forEach((step, idx) => {
			if (step == null) {
				return;
			}
			step.number = idx + 1;
